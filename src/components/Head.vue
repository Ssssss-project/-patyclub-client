<template>
    <header id="Head" :class="isHomepage ? 'home_bg' : 'normalbg'">
        <router-link :to="`/`">
        ...
        <div>
            ...
            <button :class="isHomepage ? 'btns-home' : 'btns'" @click="loginChick">創建活動</button>
            <!-- </router-link> -->
    ...
    <q-dialog v-model="bShowChat" :position="position">
        <q-card style="width: 350 px">
            <q-card-section class="column items-center no-wrap">
                ...
                <q-input v-model="sendText" label="訊息" />
...
export default {
    setup() {
            ...
            $q.dialog({
                ...
                apiGetUserProfile({ userAccount: decoded.account }).then((profileResponse) => {
                    ...
                    if (personalInfo.value.headStickerPath == "") {
                        personalInfo.value.headStickerPath = "img:" + require(`@/assets/icon/numberOfPeople.svg`);
                    } else {
        ...
        const hubConnection = ref(
            new signalR.HubConnectionBuilder().withUrl("https://localhost:5001/hub/test").build()
        );
        const sendText = ref("");
        const sendFrom = ref("");
